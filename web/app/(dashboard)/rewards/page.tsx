        ...
        <div className="space-y-5">
          ...
          {showBadgeForm && (
            <div className="bg-white rounded-xl border border-gray-200 p-5">
              <h3 className="font-semibold text-gray-900 mb-4">Nueva insignia</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
                <div className="md:col-span-1">
                  <label className="block text-xs font-medium text-gray-500 mb-1">Icono (emoji)</label>
                  <input
                    value={badgeForm.iconUrl}
                    onChange={e => setBadgeForm(f => ({ ...f, iconUrl: e.target.value }))}
                    placeholder="🏅"
                    className="w-full border border-gray-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blood-500 text-center text-2xl"
                  />
                </div>
                <div>
                  <label className="block text-xs font-medium text-gray-500 mb-1">Nombre *</label>
              ...
              <div className="flex gap-2 mt-4">
                ...
                <button onClick={createBadge} disabled={!badgeForm.name || savingBadge}
                  className="flex items-center gap-2 px-4 py-2 bg-blood-600 hover:bg-blood-700 disabled:opacity-60 text-white text-sm font-medium rounded-lg transition-colors">
                  {savingBadge
          ...
          {loadingBadges ? (
            ...
            <div className="bg-white rounded-xl border border-dashed border-gray-300 p-16 flex flex-col items-center text-center">
              ...
              <h2 className="text-lg font-semibold text-gray-500">Sin insignias creadas</h2>
              <p className="text-sm text-gray-400 mt-2">Crea insignias para reconocer los hitos de tus donantes.</p>
            </div>
          ) : (
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {badges.map(badge => (
                <div key={badge.id} className="bg-white rounded-xl border border-gray-200 p-5">
                  <div className="flex items-start gap-3">
                    <div className="w-12 h-12 bg-blood-50 rounded-xl flex items-center justify-center text-2xl flex-shrink-0">
                      {badge.iconUrl || '🏅'}
                    </div>
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs text-gray-400 mt-2">
                        <span className="font-medium text-gray-700">{badge._count.donorBadges}</span> donantes la han ganado
                      </p>
                    </div>
