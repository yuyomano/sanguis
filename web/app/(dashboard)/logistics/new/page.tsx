  ...
  return (
    <div className="p-8 max-w-4xl">
      <div className="flex items-center gap-4 mb-8">
        <button onClick={() => router.back()} className="p-2 rounded-md text-muted-foreground/70 hover:bg-muted hover:text-muted-foreground transition-colors">
          <ArrowLeft size={20} />
        </button>
        <div>
          <h1 className="font-display text-2xl font-semibold text-foreground">Nueva orden de entrega</h1>
          <p className="text-muted-foreground text-sm mt-0.5">Registra el despacho de unidades de sangre</p>
        </div>
      </div>
      ...
      <form onSubmit={handleSubmit} className="space-y-8">
        ...
        <div className="bg-card rounded-md border border-border p-6">
          <h2 className="font-semibold text-foreground mb-4">Datos del destinatario</h2>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">
                Nombre del destino <span className="text-alert">*</span>
              </label>
              <input
                required
            ...
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">
                Dirección <span className="text-alert">*</span>
              </label>
              <input
                required
        ...
        {/* Transporte */}
        <div className="bg-card rounded-md border border-border p-6">
          <h2 className="font-semibold text-foreground mb-4">Tipo de transporte</h2>
          <div className="flex gap-4 mb-4">
            {[
              { value: 'OWN', label: 'Flota propia' },
              { value: 'THIRD_PARTY', label: 'Transportista externo' },
            ].map(({ value, label }) => (
              <label key={value} className={`flex-1 flex items-center gap-3 p-4 rounded-md border-2 cursor-pointer transition-colors ${form.carrierType === value ? 'border-primary bg-blood-50' : 'border-border hover:border-input'}`}>
                <input
                  type="radio"
                  name="carrierType"
                  value={value}
          ...
          {form.carrierType === 'OWN' && (
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">Vehículo <span className="text-alert">*</span></label>
              <select
              ...
              {vehicles.length === 0 && (
                <p className="text-xs text-muted-foreground/70 mt-1">No hay vehículos activos registrados</p>
              )}
            </div>
          )}
          ...
          {form.carrierType === 'THIRD_PARTY' && (
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">Nombre del transportista <span className="text-alert">*</span></label>
              <input
          ...
          {protocols.length > 0 && (
            <div className="mt-4">
              <label className="block text-sm font-medium text-foreground mb-1">Protocolo de entrega</label>
              <select
        ...
        <div className="bg-card rounded-md border border-border p-6">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-foreground">Unidades de sangre</h2>
            {selectedIds.size > 0 && (
              <span className="text-sm font-medium text-primary bg-blood-50 px-3 py-1 rounded-full">
          ...
          {selectedUnits.length > 0 && (
            <div className="flex flex-wrap gap-2 mb-4">
              {selectedUnits.map((u) => (
                <span key={u.id} className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-blood-50 border border-blood-200 rounded-md text-xs font-medium text-blood-700">
                  <span className="font-bold">{BLOOD_LABELS[u.bloodType] || u.bloodType}</span>
                  {u.bagNumber}
                  <button type="button" onClick={() => toggleUnit(u.id)} className="ml-1 text-blood-400 hover:text-primary">
                    <X size={12} />
                  </button>
                </span>
              ))}
          ...
          <div className="relative mb-3">
            ...
            <input
              ...
              className="w-full pl-9 pr-3 py-2 border border-input rounded-md text-sm outline-none focus:ring-2 focus:ring-primary"
            />
          </div>
          ...
          {availableUnits.length === 0 ? (
            ...
            <div className="overflow-hidden rounded-md border border-border max-h-72 overflow-y-auto">
              <table className="w-full text-sm">
                <thead className="bg-muted/50 sticky top-0">
                  <tr>
                    <th className="w-10 px-4 py-2"></th>
                    <th className="text-left px-3 py-2 text-xs font-semibold text-muted-foreground uppercase">Tipo</th>
                    <th className="text-left px-3 py-2 text-xs font-semibold text-muted-foreground uppercase">Bolsa</th>
                    <th className="text-left px-3 py-2 text-xs font-semibold text-muted-foreground uppercase">Producto</th>
                    <th className="text-left px-3 py-2 text-xs font-semibold text-muted-foreground uppercase">Donante</th>
                    <th className="text-left px-3 py-2 text-xs font-semibold text-muted-foreground uppercase">Ubicación</th>
                  </tr>
                </thead>
                  ...
                  {filteredUnits.map((unit) => {
                    ...
                    return (
                      <tr
                        ...
                        className={`cursor-pointer transition-colors ${checked ? 'bg-blood-50' : 'hover:bg-muted/50'}`}
                      >
                        ...
                        <td className="px-3 py-2.5 font-mono text-xs text-foreground">{unit.bagNumber}</td>
                        <td className="px-3 py-2.5 text-muted-foreground">{PRODUCT_LABELS[unit.productType] || unit.productType}</td>
                        <td className="px-3 py-2.5 text-muted-foreground">{unit.donor?.name || '—'}</td>
                        <td className="px-3 py-2.5 text-muted-foreground">{unit.storageLocation?.name || '—'}</td>
                      </tr>
                    )
                  })}
                </tbody>
        ...
        {/* Costos */}
        <div className="bg-card rounded-md border border-border p-6">
          <h2 className="font-semibold text-foreground mb-4">Costos (opcional)</h2>
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">Costo base (RD$)</label>
              <input
                type="number"
                min="0"
                step="0.01"
            ...
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">Última milla (RD$)</label>
              <input
                type="number"
                min="0"
                step="0.01"
        ...
        {error && (
          <div className="bg-alert/5 border border-alert/30 text-alert px-4 py-3 rounded-md text-sm">
            {error}
          </div>
        )}
        ...
        <div className="flex gap-3 justify-end pb-8">
          <button
            type="button"
            onClick={() => router.back()}
            className="px-6 py-2.5 border border-input text-foreground rounded-md text-sm font-medium hover:bg-muted/50 transition-colors"
          >
            Cancelar
          </button>
          <button
            type="submit"
            disabled={submitting}
            className="px-6 py-2.5 bg-primary text-white rounded-md text-sm font-medium hover:bg-blood-600 transition-colors disabled:opacity-60"
          >
